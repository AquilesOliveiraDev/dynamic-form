...
import { createDynamicForm } from '../src/lib/dynamicForm.svelte';
import schemaData from '../src/form-schema.json';

describe('createDynamicForm Svelte 5 - 10 Regras do DynamicField', () => {
  let form: ReturnType<typeof createDynamicForm>;
  ...
  it('deve inicializar o formulário e carregar valores padrão', () => {
    expect(form.state).toBeTruthy();
    expect(form.state.values['person_type']).toBe('PF');
    expect(form.state.values['state_id']).toBe('SP');
  });

  it('1. dependentFields & 7. clearedFieldsChanged', () => {
    expect(form.state.visibility['cpf']).toBe(true);
    expect(form.state.visibility['cnpj']).toBe(false);

    // Preenche CPF
    form.setValue('cpf', '123.456.789-00');
    expect(form.state.values['cpf']).toBe('123.456.789-00');

    // Ao mudar para PJ, clearedFieldsChanged limpa CPF e exibe CNPJ
    form.setValue('person_type', 'PJ');
    expect(form.state.values['cpf']).toBeNull();
    expect(form.state.visibility['cpf']).toBe(false);
    expect(form.state.visibility['cnpj']).toBe(true);
  });

  it('2. disabledFields & 3. disabledFieldsCondition (and / or)', () => {
    // standard + PF -> payment_terms habilitado
    expect(form.state.disabledState['payment_terms']).toBe(false);
  ...
  it('5. autoSetFields & 10. calcFields em cascata', () => {
    ...
    expect(form.state.values['product_sku']).toBe('DELL-XPS-15');
    expect(form.state.values['unit_price']).toBe(8500);
    expect(form.state.values['total_price']).toBe(15300);

    // Selecionar Monitor LG (Preço 2400)
    // 2 * 2400 * (1 - 0.10) = 4320
    form.setValue('product_id', 'prod_lg');
    expect(form.state.values['product_sku']).toBe('LG-34-UW');
    expect(form.state.values['unit_price']).toBe(2400);
    expect(form.state.values['total_price']).toBe(4320);
  });
  ...
  it('8. requiredFields & 9. requiredRule com operador and', async () => {
    ...
    expect(form.state.errors['warranty_months']).toBeTruthy();
    ...
    expect(form.state.errors['warranty_months']).toBeFalsy();
  });
});
