...
class TokenInterpolator {
    ...
    static resolveToken(token, values) {
        ...
        if (token.includes('|')) {
            ...
            if (!(0, index_js_1.isFilled)(rootObj))
                return undefined;
            targetValue = (0, index_js_1.getByPath)(rootObj, subPath);
            if (targetValue === undefined && !subPath.startsWith('record.')) {
                targetValue = (0, index_js_1.getByPath)(rootObj, `record.${subPath}`);
            }
            if (targetValue === undefined && !subPath.startsWith('attributes.')) {
                targetValue = (0, index_js_1.getByPath)(rootObj, `attributes.${subPath}`);
            }
        }
        else {
        ...
        if (Array.isArray(targetValue)) {
            return targetValue.map((item) => (0, index_js_1.normalizeComparableValue)(item));
        }
        return (0, index_js_1.normalizeComparableValue)(targetValue);
    }
    /**
     ...
     */
    static interpolateString(template, values) {
     ...
     */
    static interpolateSerializable(data, values) {
...
exports.TokenInterpolator = TokenInterpolator;
class ArithmeticParser {
     ...
     */
    static evaluate(expression, values, step) {
        ...
        try {
            ...
            const decimalPlaces = Number.isFinite(parsedStep) && parsedStep > 0
    ...
    static parseArithmetic(input) {
        ...
        const parseNumber = () => {
            ...
            while (pos < input.length && /[0-9.]/.test(input[pos]))
                pos++;
            if (pos === start)
                throw new Error(`Token inesperado na posição ${pos}`);
        ...
        if (pos !== input.length)
            throw new Error(`Caractere inesperado na posição ${pos}`);
        return result;
    }
}
