...
const index_js_2 = require("../expression/index.js");
class RuleEvaluator {
    /**
    ...
    static ruleValueMatches(rawValue, expected) {
        if (expected === 'any')
            return (0, index_js_1.isFilled)(rawValue);
        const normalized = (0, index_js_1.normalizeComparableValue)(rawValue);
    ...
    static evaluateRule(rule, values, operator = 'or') {
        ...
        if (Array.isArray(rule)) {
            return operator === 'and'
                ? rule.every((fieldName) => (0, index_js_1.isFilled)(values[fieldName]))
                : rule.some((fieldName) => (0, index_js_1.isFilled)(values[fieldName]));
        }
    ...
    static evaluateOperator(targetValue, expectedValue, operator = '===') {
        const normTarget = (0, index_js_1.normalizeComparableValue)(targetValue);
        const normExpected = (0, index_js_1.normalizeComparableValue)(expectedValue);
        switch (operator) {
            case '===':
            case '==':
            ...
            case 'includes':
                ...
                return false;
            case 'isFilled':
                return (0, index_js_1.isFilled)(normTarget);
            case 'isEmpty':
                return !(0, index_js_1.isFilled)(normTarget);
            default:
                return String(normTarget ?? '') === String(normExpected ?? '');
        ...
        const results = entries.map((entry) => {
            if (entry.key) {
                ...
                if (typeof areaId === 'string' && areaId.includes('#{')) {
                    areaId = index_js_2.TokenInterpolator.resolveToken(areaId.replace(/#\{|\}#/g, ''), values);
                }
                let currentSetting;
