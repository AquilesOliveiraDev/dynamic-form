...
export class DependencyGraph {
  private dependencies: Map<string, FieldDependencyMap> = new Map();
  private fieldsByName: Map<string, DynamicField> = new Map();
  private dualPairs: Set<string> = new Set(); // source:dependent
  ...
  public getField(name: string): DynamicField | undefined {
    return this.fieldsByName.get(name);
  }

  public getDependencies(name: string): FieldDependencyMap | undefined {
    return this.dependencies.get(name);
  }

  public isDualPair(sourceField: string, dependentField: string): boolean {
    return this.dualPairs.has(`${sourceField}:${dependentField}`);
  }

  private ensureNode(name: string): FieldDependencyMap {
  ...
  private buildGraph(matrix: DynamicField[][]): void {
    const flatFields: DynamicField[] = [];
    ...
    for (const field of flatFields) {
      ...
      if (field.clearedFields) {
        const isChangedMode = Boolean(field.clearedFieldsChanged || (field.attr as any)?.clearedFieldsChanged);
        if (isChangedMode) {
      ...
      // 4. Rule Condition
      const rule = field.attr?.rule || (field as any).rule;
      if (rule) {
  ...
  private extractClearedSources(clearedFields: unknown): string[] {
    if (!clearedFields) return [];
    if (Array.isArray(clearedFields)) return clearedFields;
    if (typeof clearedFields === 'object') {
      const obj = clearedFields as Record<string, any>;
      if (obj.fields) {
   ...
   */
  public getTopologicalAffectedFields(changedField: string): {
    clears: string[];
    reloads: string[];
    conditions: string[];
    autoSets: string[];
   ...
   */
  public hasMatchingAutoSetMirror(
    sourceField: string,
    ...
    if (depField?.autoSetFields && sourceField in depField.autoSetFields) {
      ...
      if (path) {
        ...
        if (!hasValueChanged(values[dependentField], derived)) return true;
      }
    }
    ...
    if (srcField?.autoSetFields && dependentField in srcField.autoSetFields) {
      ...
      if (path) {
        ...
        if (!hasValueChanged(values[sourceField], derived)) return true;
      }
    }
